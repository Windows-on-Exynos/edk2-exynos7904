...
DisableWtsr ()
{
  ...
  Status = mSpeedyProtocol->Read (0x11A10000, S2MPU09_RTC_ADDR,
  ...
  Status = mSpeedyProtocol->Write (0x11A10000, S2MPU09_RTC_ADDR,
                                    S2MPU09_RTC_WTSR_SMPL, Value);
  if (EFI_ERROR (Status)) {
...
DisplayPmicInfos ()
{
  ...
  Status = mSpeedyProtocol->Read (0x11A10000, S2MPU09_PM_ADDR,
  ...
  if (!EFI_ERROR (Status)) {
    DEBUG ((EFI_D_WARN, "S2MPU09 RTC_BUFF    =0x%x\n", Value));
  }
...
InitPmic ()
{
  ...
  Status = mSpeedyProtocol->Write (0x11A10000, S2MPU09_PM_ADDR,
                                    S2MPU09_PM_CTRL1, Value);
  if (EFI_ERROR (Status)) {
