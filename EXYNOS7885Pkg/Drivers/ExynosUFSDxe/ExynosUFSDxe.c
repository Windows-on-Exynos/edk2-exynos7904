...
static UINT8 ufs_query_params[][5] = {
 ...
 */
static inline EFI_STATUS ufs_init_cal(struct ufs_host *ufs, int idx)
{
    ...
    if (ret != UFS_CAL_NO_ERROR) {
        DEBUG((EFI_D_ERROR, "UFS Init failed with %d\n", ret));
        Status = ERR_GENERIC;
    }
...
#define ERR_GENERIC EFI_DEVICE_ERROR

static inline EFI_STATUS ufs_pre_link(struct ufs_host *ufs, UINT8 lane)
{
    EFI_STATUS Status = EFI_SUCCESS;
    int ret = 0;
    ...
    if (ret != UFS_CAL_NO_ERROR) {
        DEBUG((EFI_D_ERROR, "ufs_pre_link failed with %d!!!\n", ret));
        Status = ERR_GENERIC;
    }
...
static inline EFI_STATUS ufs_post_link(struct ufs_host *ufs)
{
    EFI_STATUS Status = EFI_SUCCESS;
    int ret = 0;
    ...
    if (ret != UFS_CAL_NO_ERROR) {
        DEBUG((EFI_D_ERROR, "ufs_post_link failed with %d!!!\n", ret));
        Status = ERR_GENERIC;
    }
...
static inline EFI_STATUS ufs_pre_gear_change(struct ufs_host *ufs, struct uic_pwr_mode *pmd)
{
    EFI_STATUS Status = EFI_SUCCESS;
    struct ufs_cal_param *p = ufs->cal_param;
    int ret = 0;
    ...
    if (ret != UFS_CAL_NO_ERROR) {
        DEBUG((EFI_D_ERROR, "ufs_pre_gear_change failed with %d!!!\n", ret));
        Status = ERR_GENERIC;
    }
...
static inline EFI_STATUS ufs_post_gear_change(struct ufs_host *ufs)
{
    EFI_STATUS Status = EFI_SUCCESS;
    int ret = 0;
    ...
    if (ret != UFS_CAL_NO_ERROR) {
        DEBUG((EFI_D_ERROR, "ufs_post_gear_change failed with %d!!!\n", ret));
        Status = ERR_GENERIC;
    }
...
void ufs_lld_pma_write(void *h, UINT32 val, UINT32 addr)
{
    ...
    MmioWrite32(val, ufs->phy_pma + addr);
}
...
UINT32 ufs_lld_pma_read(void *h, UINT32 addr)
{
    ...
    return MmioRead32(ufs->phy_pma + addr);
}
...
{
    struct ufs_host *ufs = (struct ufs_host *)h;
    MmioWrite32(val, ufs->unipro_addr + addr);
}

void ufs_lld_udelay(UINT32 val)
{
    // Usar la biblioteca de temporizador de UEFI para esperar microsegundos
    MicroSecondDelay(val);
}

void ufs_lld_usleep_delay(UINT32 min, UINT32 max)
{
    // Esperar el tiempo máximo especificado en microsegundos
    MicroSecondDelay(max);
}

UINT64 ufs_lld_get_time_count(UINT64 offset)
{
    // UEFI usa UINT64 para tiempos, así que se mantiene el offset
    return offset;
}

UINT64 ufs_lld_calc_timeout(UINT32 ms)
{
	...
	if (len) {
		...
		for (i = 0; i < sg_segments; i++) {
			ufs->cmd_desc_addr->prd_table[i].size =
			    (UINT32) UFS_SG_BLOCK_SIZE - 1;
			ufs->cmd_desc_addr->prd_table[i].base_addr =
			    (UINT32)(((UINT64) (ufs->scsi_cmd->buf) + i * UFS_SG_BLOCK_SIZE) & (((UINT64)1 << UFS_BIT_LEN_OF_DWORD) - 1));
			ufs->cmd_desc_addr->prd_table[i].upper_addr =
			    (UINT32)(((UINT64) (ufs->scsi_cmd->buf) + i * UFS_SG_BLOCK_SIZE) >> UFS_BIT_LEN_OF_DWORD);
		}
	}
}

static UINT32 __utp_cmd_get_dir(scm *pscm)
{
	UINT32 data_direction;
...
static UINT32 __utp_cmd_get_flags(scm *pscm)
{
	UINT32 upiu_flags;
...
static void __utp_write_cmd_ucd(struct ufs_host *ufs)
{
	...
	memcpy(&tsf[0], &datalen, sizeof(UINT32));
...
static int __utp_write_query_ucd(struct ufs_host *ufs, query_index qry)
{
	...
	if (hdr->function == UFS_STD_WRITE_REQ) {
		...
		memcpy(&tsf[6], &info, sizeof(UINT16));
	...
	if (tsf[0] == UPIU_QUERY_OPCODE_WRITE_ATTR) {
		...
		memcpy(&tsf[8], &info, sizeof(UINT32));
	} else if (tsf[0] == UPIU_QUERY_OPCODE_SET_FLAG)
		tsf[11] = (UINT8)ufs->flags.arry[tsf[1]];
...
static int __utp_write_utrd(struct ufs_host *ufs, UINT32 type)
{
	int r = 0;

	struct ufs_utrd *utrd_ptr = ufs->utrd_addr;
	UINT32 len = ufs->scsi_cmd->datalen;
	UINT16 sg_segments = (UINT16)((len + UFS_SG_BLOCK_SIZE - 1) / UFS_SG_BLOCK_SIZE);
	...
	case UPIU_TRANSACTION_COMMAND:
		...
		utrd_ptr->dw[0] = (UINT32)(data_direction | UTP_SCSI_COMMAND | UTP_REQ_DESC_INT_CMD);
		utrd_ptr->dw[2] = (UINT32)(OCS_INVALID_COMMAND_STATUS);
		if (len) {
 ...
 */
static int handle_ufs_uic_int(struct ufs_host *ufs, UINT32 intr_stat)
{
...
static int handle_ufs_utp_int(struct ufs_host *ufs, UINT32 intr_stat)
{
	int ret = UFS_IN_PROGRESS;

	if (intr_stat & UTP_TRANSFER_REQ_COMPL) {
		if (!(MmioRead32(ufs->ioaddr + REG_UTP_TRANSFER_REQ_DOOR_BELL) & 1))
			ret = UFS_NO_ERROR;
	}
...
int handle_ufs_int(struct ufs_host *ufs, int is_uic)
{
	...
	intr_stat = MmioRead32(ufs->ioaddr + REG_INTERRUPT_STATUS);
...
static int send_uic_cmd(struct ufs_host *ufs)
{
	...
	MmioWrite32(ufs->uic_cmd->uiccmdarg1, (ufs->ioaddr + REG_UIC_COMMAND_ARG_1));
	MmioWrite32(ufs->uic_cmd->uiccmdarg2, (ufs->ioaddr + REG_UIC_COMMAND_ARG_2));
	MmioWrite32(ufs->uic_cmd->uiccmdarg3, (ufs->ioaddr + REG_UIC_COMMAND_ARG_3));
	MmioWrite32(ufs->uic_cmd->uiccmdr, (ufs->ioaddr + REG_UIC_COMMAND));

	ufs->timeout = ufs->uic_cmd_timeout;	/* usec unit */
	while (UFS_IN_PROGRESS == (err = handle_ufs_int(ufs, 1)));
	MmioWrite32(MmioRead32(ufs->ioaddr + REG_INTERRUPT_STATUS),
			ufs->ioaddr + REG_INTERRUPT_STATUS);

	error_code = MmioRead32(ufs->ioaddr + REG_UIC_COMMAND_ARG_2);
	if (err || error_code) {
		printf("UFS(%d) UIC command error!\n\t%08x %08x %08x %08x\n\t%08x %08x %08x %08x\n",
			...
			MmioRead32(ufs->ioaddr + REG_UIC_COMMAND),
			MmioRead32(ufs->ioaddr + REG_UIC_COMMAND_ARG_1),
			MmioRead32(ufs->ioaddr + REG_UIC_COMMAND_ARG_2),
			MmioRead32(ufs->ioaddr + REG_UIC_COMMAND_ARG_3));
			exynos_ufs_get_uic_info(ufs);
	}
	if (ufs->uic_cmd->uiccmdr == UIC_CMD_DME_GET
	    || ufs->uic_cmd->uiccmdr == UIC_CMD_DME_PEER_GET) {
		ufs->uic_cmd->uiccmdarg3 = MmioRead32(ufs->ioaddr + REG_UIC_COMMAND_ARG_3);
#ifdef SCSI_UFS_DEBUG
		printf
...
static void __utp_send(struct ufs_host *ufs, UINT32 type)
{

	switch (type) {
	case UPIU_TRANSACTION_NOP_OUT:
	case UPIU_TRANSACTION_QUERY_REQ:
		MmioWrite32(0x0, (ufs->vs_addr + VS_UTRL_NEXUS_TYPE));
		break;
	case UPIU_TRANSACTION_COMMAND:
		MmioWrite32(0xFFFFFFFF, (ufs->vs_addr + VS_UTRL_NEXUS_TYPE));
	default:
		break;
	}
	...
	case UPIU_TRANSACTION_COMMAND:
	case UPIU_TRANSACTION_QUERY_REQ:
		MmioWrite32(1, (ufs->ioaddr + REG_UTP_TRANSFER_REQ_DOOR_BELL));
		break;
	default:
		break;
	}
...
static int __utp_wait_for_response(struct ufs_host *ufs, UINT32 type)
{
	...
	if (type == UPIU_TRANSACTION_NOP_OUT || type == UPIU_TRANSACTION_QUERY_REQ)
		MmioWrite32((MmioRead32(ufs->ioaddr + 0x140) | 0x01), (ufs->ioaddr + 0x140));

	return err;
}

static void __utp_init(struct ufs_host *ufs, UINT32 lun)
{
	ufs->lun = lun;
	ufs->scsi_cmd = NULL;
...
static void __utp_query_read_info(struct ufs_host *ufs, UINT8 idn)
{
	struct ufs_upiu *resp_ptr = &ufs->cmd_desc_addr->response_upiu;
	UINT8 *data = resp_ptr->data;
	void *dst = NULL;
	size_t len = 0;
	int lun;
...
{
	struct ufs_upiu *resp_ptr = &ufs->cmd_desc_addr->response_upiu;
	UINT8 opcode = ufs_query_params[qry][1];
	...
	case UPIU_QUERY_OPCODE_READ_ATTR:
		tsf = resp_ptr->tsf;
		val = UPIU_HEADER_DWORD((UINT32) tsf[8], (UINT32) tsf[9], (UINT32) tsf[10], (UINT32) tsf[11]);
...
{
	int r;
	UINT32 type = UPIU_TRANSACTION_COMMAND;
...
static int ufs_utp_cmd_process_swp(struct ufs_host *ufs, scm * pscm, UINT32 lun)
{
	int r;
	UINT32 type = UPIU_TRANSACTION_COMMAND;
...
{
	int r;
	UINT32 type = UPIU_TRANSACTION_NOP_OUT;
 ...
 */
static int ufs_utp_query_process(struct ufs_host *ufs, query_index qry, UINT32 lun)
{
	int r;
	UINT32 type = UPIU_TRANSACTION_QUERY_REQ;
...
static int ufs_utp_query_retry(struct ufs_host *ufs, query_index qry, UINT32 lun)
{
	int r = 0;
	int retries;
 ...
 */
int ufs_edit_config_desc(UINT32 lun, UINT32 enable, UINT32 bootlun, UINT32 writeprotect, UINT32 type, UINT32 capacity)
{
	int i;
	UINT32 sum = 0;
	int res = 0;
...
static int ufs_mphy_unipro_setting(struct ufs_host *ufs, struct ufs_uic_cmd *uic_cmd_list)
{
    ...
    if (!uic_cmd_list) {
        DEBUG((EFI_D_WARN, "%s: cmd list is empty\n", __func__));
        return res;
    }
    ...
    while (uic_cmd_list->uiccmdr) {
        ...
        case UIC_CMD_WAIT:
            gBS->Stall(ufs->uic_cmd->uiccmdarg2);  // Sustituye u_delay por Stall (microsegundos)
            break;

        case UIC_CMD_WAIT_ISR:
            timeout = ufs->uic_cmd_timeout;
            while ((MmioRead32(ufs->ioaddr + ufs->uic_cmd->uiccmdarg1) &
                ufs->uic_cmd->uiccmdarg2) != ufs->uic_cmd->uiccmdarg2) {
                if (!timeout--) {
                    res = 0;
                    goto out;
                }
                gBS->Stall(1);  // Reemplaza u_delay(1) por Stall(1 microsegundo)
            }
            MmioWrite32(ufs->uic_cmd->uiccmdarg2, (ufs->ioaddr + ufs->uic_cmd->uiccmdarg1));
            break;

        case UIC_CMD_REGISTER_SET:
            MmioWrite32(ufs->uic_cmd->uiccmdarg2, (ufs->unipro_addr + ufs->uic_cmd->uiccmdarg1));
            break;

        case PHY_PMA_COMN_SET:
        case PHY_PMA_TRSV_SET:
            MmioWrite32(ufs->uic_cmd->uiccmdarg2, (ufs->phy_pma + ufs->uic_cmd->uiccmdarg1));
            break;

        case PHY_PMA_COMN_WAIT:
        case PHY_PMA_TRSV_WAIT:
            timeout = ufs->uic_cmd_timeout;
            while ((MmioRead32(ufs->phy_pma + ufs->uic_cmd->uiccmdarg1) &
                ufs->uic_cmd->uiccmdarg2) != ufs->uic_cmd->uiccmdarg2) {
                if (!timeout--) {
                    res = 0;
                    goto out;
        ...
        default:
            res = send_uic_cmd(ufs);  // Asegúrate de que esta función esté implementada en UEFI
            break;
        }
    }
...
out:
    if (res) {
        DEBUG((EFI_D_ERROR, "%s: failed cmd:0x%x arg1:0x%x arg2:0x%x with err %d\n",
...
static int ufs_update_max_gear(struct ufs_host *ufs)
{
    ...
    ufs->uic_cmd = &rx_cmd;
    ret = send_uic_cmd(ufs);  // Asegúrate de que esta función esté correctamente implementada
    if (ret)
        goto out;
    ...
    // Imprimir información de depuración en UEFI
    DEBUG((EFI_D_INFO, "ufs max_gear(%d)\n", p->max_gear));
...
int ufs_device_reset()
{
	...
	MmioWrite32(0, ufs->vs_addr + VS_GPIO_OUT);
	u_delay(5);
	MmioWrite32(1, ufs->vs_addr + VS_GPIO_OUT);
...
static int ufs_pre_setup(struct ufs_host *ufs)
{
	...
	val = MmioRead32(ufs->phy_iso_addr);
	if ((val & 0x1) == 0) {
		val |= (1 << 0);
		MmioWrite32(val, ufs->phy_iso_addr);
	}

	/* VS_SW_RST */
	if ((MmioRead32(ufs->vs_addr + VS_FORCE_HCS) >> 4) & 0xf)
		MmioWrite32(0x0, (ufs->vs_addr +  VS_FORCE_HCS));

	MmioWrite32(3, (ufs->vs_addr + VS_SW_RST));
	while (MmioRead32(ufs->vs_addr + VS_SW_RST))
		;

	/* VENDOR_SPECIFIC_IS[20] : clear UFS_IDLE_Indicator bit (if UFS_LINK is reset, this bit is asserted) */
	reg = MmioRead32(ufs->vs_addr + VS_IS);
	if ((reg >> 20) & 0x1)
		MmioWrite32(reg, (ufs->vs_addr + VS_IS));
	...
	if (ufs->quirks & UFS_QUIRK_BROKEN_HCE) {
		...
		MmioWrite32(1, (ufs->ioaddr + REG_CONTROLLER_ENABLE));

		while (!(MmioRead32(ufs->ioaddr + REG_CONTROLLER_ENABLE) & 0x1))
			u_delay(1);
	}

	/*ctrl refclkout*/
	MmioWrite32((MmioRead32(ufs->vs_addr + VS_CLKSTOP_CTRL) & ~(1 << 4)), ufs->vs_addr + VS_CLKSTOP_CTRL);

	/*CLOCK GATING SET*/
	MmioWrite32(0xde0, ufs->vs_addr + VS_FORCE_HCS);

	MmioWrite32(MmioRead32(ufs->vs_addr + VS_UFS_ACG_DISABLE)|1, ufs->vs_addr + VS_UFS_ACG_DISABLE);
	...
	ufs->utrd_addr->cmd_desc_addr_l = (UINT64)(ufs->cmd_desc_addr);
	ufs->utrd_addr->rsp_upiu_off = (UINT16)(offsetof(struct ufs_cmd_desc, response_upiu));
	ufs->utrd_addr->rsp_upiu_len = (UINT16)(ALIGNED_UPIU_SIZE);

	MmioWrite32((UINT64)ufs->utmrd_addr, (ufs->ioaddr + REG_UTP_TASK_REQ_LIST_BASE_L));
	MmioWrite32(0, (ufs->ioaddr + REG_UTP_TASK_REQ_LIST_BASE_H));

	MmioWrite32((UINT64)ufs->utrd_addr, (ufs->ioaddr + REG_UTP_TRANSFER_REQ_LIST_BASE_L));
	MmioWrite32(0, (ufs->ioaddr + REG_UTP_TRANSFER_REQ_LIST_BASE_H));

	// TODO: cport
	MmioWrite32(0x22, ufs->vs_addr + 0x114);
	MmioWrite32(1, ufs->vs_addr + 0x110);
...
static void ufs_vendor_setup(struct ufs_host *ufs)
{
	...
	MmioWrite32(0xa, (ufs->vs_addr + VS_DATA_REORDER));

	MmioWrite32(1, (ufs->ioaddr + REG_UTP_TASK_REQ_LIST_RUN_STOP));

	MmioWrite32(1, (ufs->ioaddr + REG_UTP_TRANSFER_REQ_LIST_RUN_STOP));

	MmioWrite32(UFS_SG_BLOCK_SIZE_BIT, (ufs->vs_addr + VS_TXPRDT_ENTRY_SIZE));
	MmioWrite32(UFS_SG_BLOCK_SIZE_BIT, (ufs->vs_addr + VS_RXPRDT_ENTRY_SIZE));

	MmioWrite32(0xFFFFFFFF, (ufs->vs_addr + VS_UTRL_NEXUS_TYPE));
	MmioWrite32(0xFFFFFFFF, (ufs->vs_addr + VS_UMTRL_NEXUS_TYPE));
}
...
static int ufs_pmc_common(struct ufs_host *ufs, struct uic_pwr_mode *pmd)
{
	...
	reg = MmioRead32(ufs->ioaddr + REG_CONTROLLER_STATUS);
...
{
    // Asignación de memoria dinámica con AllocatePool
    tempbuff = AllocatePool(0x2000);

    // Mensajes de depuración con DEBUG
    DEBUG((EFI_D_INFO, "cmd_desc_addr : %p\n", ufs->cmd_desc_addr));
    DEBUG((EFI_D_INFO, "\tresponse_upiu : %p\n", &ufs->cmd_desc_addr->response_upiu));
    DEBUG((EFI_D_INFO, "\tprd_table : %p (size=%lx)\n", ufs->cmd_desc_addr->prd_table,
           sizeof(ufs->cmd_desc_addr->prd_table)));
    DEBUG((EFI_D_INFO, "\tsizeof upiu : %lx\n", sizeof(struct ufs_upiu)));

    // Inicializa cmd_desc y utrd con memset
    memset(ufs->cmd_desc_addr, 0x00, UFS_NUTRS * sizeof(struct ufs_cmd_desc));
    memset(ufs->utrd_addr, 0x00, UFS_NUTRS * sizeof(struct ufs_utrd));

    // Configuración de los registros UTRD
    ufs->utrd_addr->cmd_desc_addr_l = (UINT64)(ufs->cmd_desc_addr);
    ufs->utrd_addr->rsp_upiu_off = (UINT16)(offsetof(struct ufs_cmd_desc, response_upiu));
    ufs->utrd_addr->rsp_upiu_len = (UINT16)(ALIGNED_UPIU_SIZE);
...
{
	MmioWrite32(0x0, ufs->fmp_addr + UFSP_UPSBEGIN0);
	MmioWrite32(0xffffffff, ufs->fmp_addr + UFSP_UPSEND0);
	MmioWrite32(0xff, ufs->fmp_addr + UFSP_UPLUN0);
	MmioWrite32(0xf1, ufs->fmp_addr + UFSP_UPSCTRL0);
}
...
static int ufs_init_host(int host_index, struct ufs_host *ufs)
{
	...
	ufs->capabilities = MmioRead32(ufs->ioaddr + REG_CONTROLLER_CAPABILITIES);
	ufs->ufs_version = MmioRead32(ufs->ioaddr + REG_UFS_VERSION);
	...
	ufs_debug("%s\n\tcaps(0x%p) 0x%08x\n\tver(0x%p)  0x%08x\n\tPID(0x%p)  0x%08x\n\tMID(0x%p)  0x%08x\n",
		...
		MmioRead32(ufs->ioaddr + REG_CONTROLLER_PID),
		ufs->ioaddr + REG_CONTROLLER_MID, MmioRead32(ufs->ioaddr + REG_CONTROLLER_MID));
...
{
	int i, res;
	UINT32 capacity, alloc_unit, value;
	struct ufs_host *ufs = get_cur_ufs_host();
 ...
 */
EFI_STATUS ufs_set_configuration_descriptor(void)
{
    ...
    // Obtiene el host UFS actual (puedes necesitar ajustar esto en UEFI)
    struct ufs_host *ufs = get_cur_ufs_host(); // Verifica cómo obtienes esta referencia en UEFI

    while (ufs_check_config_desc() == RET_FAILURE) {
        if (retry_count == retry) {
            DEBUG((EFI_D_ERROR, "UFS LU config: FAIL !!!\n"));
            print_ufs_information();  // Necesitarás implementar esta función en UEFI si es necesario
            break;
        }
        DEBUG((EFI_D_INFO, "UFS LU config: trying %d...\n", retry++));
        ...
        for (lun = 7; lun >= 0; lun--) {
            ...
            if (EFI_ERROR(ret)) {
                DEBUG((EFI_D_ERROR, "UFS configuration setup failed with error: %d\n", ret));
        ...
        if (EFI_ERROR(ret)) {
            DEBUG((EFI_D_ERROR, "UFS LU config: Descriptor write query failed with %d\n", ret));
            continue;
        }

        // Configuración del atributo BootLUN
        ufs->attributes.arry[UPIU_ATTR_ID_BOOTLUNEN] = 0x01;
        ret = ufs_utp_query_retry(ufs, ATTR_W_BOOTLUNEN, 0);
        if (EFI_ERROR(ret)) {
            DEBUG((EFI_D_ERROR, "UFS LU config: BootLUNEN setting failed with %d\n", ret));
            continue;
        }

        ret = ufs_utp_query_retry(ufs, ATTR_R_BOOTLUNEN, 0);
        if (EFI_ERROR(ret)) {
            DEBUG((EFI_D_ERROR, "UFS LU config: BootLUNEN read failed with %d\n", ret));
            continue;
        }

        // Pausa de 1 segundo en UEFI
        gBS->Stall(1000000); // 1000 ms (1 segundo)
    }

    // Si la configuración tuvo éxito
    if (retry) {
        DEBUG((EFI_D_INFO, "UFS LU config: PASS !!!\n"));

        // Remueve dispositivos enumerados, ajusta scsi_exit() a UEFI si es necesario
...
EFI_STATUS ufs_init(int mode)
{
    EFI_STATUS r = EFI_SUCCESS;
    int i;
    int rst_cnt = 0;

    ufs_lu_list.prev = NULL;
    ufs_lu_list.next = NULL;

    // Initialize Logical Unit count
    ...
    for (i = 0; i < SCSI_MAX_INITIATOR; i++) {
        ...
        do {
            ...
            if (!EFI_ERROR(r))
                break;
            rst_cnt++;
            DEBUG((EFI_D_ERROR, "UFS: Retry Link Startup CNT : %d\n", rst_cnt));
        } while (rst_cnt < 5);
        ...
        r = scsi_scan(ufs_dev[i], 0, ufs_number_of_lus, scsi_exec, NULL, 128, &ufs_lu_list);
        if (EFI_ERROR(r))
            goto out;

        r = scsi_scan(&ufs_dev_rpmb, 0x44, 0, scsi_exec, "rpmb", 128, &ufs_lu_list);
        if (EFI_ERROR(r))
            goto out;

        r = scsi_scan_ssu(&ufs_dev_ssu, 0x50, scsi_exec, (get_sdev_t *)scsi_get_ssu_sdev, &ufs_lu_list);
        if (EFI_ERROR(r))
            goto out;
...
out:
    _ufs_curr_host = 0;  // Assume host #0 by default
    return r;
}
...
void sw_lock(UINT32 lun, UINT32 set)
{
	scm *pscm = (scm *)malloc(sizeof(struct scsi_command_meta));
	struct ufs_host *ufs;
	int res;
	DEBUG((EFI_D_INFO, "UFS SWP SWP sw_lock: lun: %u\n", lun));

	UINT8 page[]={0x00,0x12,0x00,0x10, 0x00, 0x00, 0x00, 0x00,
	...
	res = ufs_utp_cmd_process_swp(ufs, pscm, lun);
	if (res) {
		DEBUG((EFI_D_ERROR, "UFS SW Lock Fail : %d\n", res));
		return;
	}

	if (set ==1)
		DEBUG((EFI_D_INFO, "UFS SW lock success\n"));
	else if (set ==0)
		DEBUG((EFI_D_INFO, "UFS SW unlock success\n"));
}
...
int ufs_alloc_memory()
{
	...
	for (i = 0; i < SCSI_MAX_INITIATOR; i++) {
		...
		if (!(_ufs[i] = malloc(len)))
			goto end;
		ufs = _ufs[i];
		SetMem(_ufs[i], 0x00, sizeof(struct ufs_host));
		...
		if (!(ufs->cmd_desc_addr = memalign(0x1000, len))) {
			DEBUG((EFI_D_ERROR, "UFS: %s: cmd_desc_addr memory alloc error!!!\n", __func__));
			goto end;
		}
		if ((UINT64)(ufs->cmd_desc_addr) & 0xfff) {
			DEBUG((EFI_D_ERROR, "UFS: %s: allocated cmd_desc_addr memory align error!!!\n", __func__));
			goto end;
		}

		len = UFS_NUTRS * sizeof(struct ufs_utrd);
		if (!(ufs->utrd_addr = memalign(0x1000, len))) {
			DEBUG((EFI_D_ERROR, "UFS: %s: utrd_addr memory alloc error!!!\n", __func__));
			goto end;
		}
		if ((UINT64)(ufs->utrd_addr) & 0xfff) {
			DEBUG((EFI_D_ERROR, "UFS: %s: allocated utrd_addr memory align error!!!\n", __func__));
			goto end;
		}
		...
		if (!(ufs_dev[i] = (scsi_device_t *)malloc(len))) {
			DEBUG((EFI_D_ERROR, "UFS: %s: ufs_dev memory allocation failed\n", __func__));
			goto end;
		}
	}
	...
	if (r != 0)
		DEBUG((EFI_D_ERROR, "##### UEFI memory allocation fails !!! #####\n"));
